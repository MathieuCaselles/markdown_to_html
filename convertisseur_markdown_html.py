...
parser.add_argument(
    "-o", "--output-directory", help="Le dossier de destination", type=str
)
parser.add_argument(
    "-a",
    "--achtung",
...
parser.add_argument(
    "-k",
    "--kikoo_lol",
    help="Activer l'option permet d'ajouter dans le texte des « kikoo », « lol », « mdr », « ptdr » de manière aléatoire",
    action="store_true",
)
...
if arg.input_directory == None or arg.output_directory == None:
    print(
        "IL vous manque quelque chose: Utilisez «python convertisseur_markdown_html.py -h» ou sinon vous pouvez aussi vous référer au readme"
    )
...
def achtung(texte):
    ...
    texte = texte.replace("htpz", "https")
...
def kikoo(texte):
    ...
    for i in range(int(len(texte) / 50)):

        ajout = randint(0, len(texte))
        mot = choice(["kikoo", "lol", "mdr", "ptdr"])
        texte = texte[:ajout] + mot + texte[ajout:]
...
for fichier_a_convertir in dossier_1:
    ...
    if fichier_a_convertir.endswith(".md"):
        ...
        if arg.kikoo_lol:
            mon_fichier = kikoo(mon_fichier)

        with open(dossier_2 + "/" + nouveau_nom + ".html", "x") as fichier:
            fichier.write(debut_html)
            fichier.write(
                markdown2.markdown(mon_fichier)
            )  # markdown2.markdown() fait la conversion automatiquement ! C'est super tchité !
